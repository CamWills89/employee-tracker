...
const addEmployee = () => {
  inquirer
    ...
    .then((res) => {
      var roleId = employeeRoles().indexOf(res.role) + 1
      var managerId = managers().indexOf(res.manager) + 1
  ...
  inquirer
    ...
    .then((res) => {
      ...
      connection.query(
        ...
        function (err) {
          if (err) throw err;
          console.log(`${res.role} role was succesfully added. \n`);
          mainMenu();
        }
      );
    });
  ...
  connection.query(
    ...
    function (err, res) {
      ...
      res.forEach((role) => {
        ...
        // managerArr.push([role.first_name, role.last_name, role.manager_id]);
        managerArr.push(role.first_name);
        // managerArr.push(role.id);
